...
\title{sorvi\\ty{\"o}kaluja avoimelle datalle}
...
'geos-dev', 'libgeos-dev' riippuvuuksineen ja manuaalisesti PROJ.4
...
<<ins,results=hide,eval=FALSE>>=
install.packages(c("methods", "ggplot2", "gpclib", "mapproj", "maps", "maptools", "pxR", "RCurl", "ReadImages", "RgoogleMaps", "rjson", "sp", "XML"))
@

Muita hyödyllisiä paketteja, joita osa esimerkeistä käyttää ja joita
kannattaa asentaa tarpeen mukaan ovat
<<ins2,results=hide,eval=FALSE>>=
install.packages(c("gdata", "plyr", "png", "rgdal", "rgeos"))
@


Nyt sorvin pitäisi asentua R:ssä:
<<initial2,results=hide,eval=FALSE>>= 
...
file.name <- "extdata/Maanmittauslaitos/1-milj-Shape-etrs-shape/forest.shp"
shape.file <- system.file(file.name, package = "sorvi")  
...
vaestotiheys <- hae.maakuntatiedot(url)
@
